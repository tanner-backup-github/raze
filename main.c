...
#define X 0.5
	GLfloat vertices[] = {X,  X,  1.0, 1.0, X,  -X, 1.0, 0.0,
			      -X, -X, 0.0, 0.0, -X, X,  0.0, 1.0};
	GLuint indices[] = {
		0, 1, 3, 1, 2, 3,
	};
	GLuint VBO = make_buffer(GL_ARRAY_BUFFER, vertices, sizeof(vertices));
	GLuint IBO =
		make_buffer(GL_ELEMENT_ARRAY_BUFFER, indices, sizeof(indices));
	...
	Font *f = new_font(ft, "razefiles/ComicNeue-Regular.otf", 64);

	int32_t w = 0, h = 0;
	GLuint tex_id = generate_text("Some test text", f, &w, &h);

	int32_t w2 = 0, h2 = 0;
	GLuint tex_id2 = generate_text("Same font, different color", f, &w2, &h2);
	...
	while (!glfwWindowShouldClose(window)) {
...

		{
			...
			model_matrix(modelm, WINDOW_WIDTH / 2 - w / 2, WINDOW_HEIGHT / 2 - h / 2, w, h,
				     0);
			...
			glUniform4f(glGetUniformLocation(shader, "color"), 0.7,
				    0.3, 0.8, 1.0);
		...
		{
			GLfloat modelm[] = IDENTITY_MATRIX;
			model_matrix(modelm, WINDOW_WIDTH / 2 - w2 / 2, h + WINDOW_HEIGHT / 2 - h2 / 2, w2,
				     h2, 0);
			...
			glUniform4f(glGetUniformLocation(shader, "color"), 0.9,
				    0.3, 0.1, 1.0);
